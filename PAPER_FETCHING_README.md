...
- 服务端最多接收 100 篇周报候选。
- 如果开启原文分析，服务端会优先抓取 arXiv HTML 页面，清洗并截取作者机构、摘要、方法、系统、实验、结果、局限和结论等关键段落。
- 如果没有可用 HTML 原文，会使用摘要和原始分析作为复评依据，并在提示中保留这个证据边界。
- LLM 会重新给出研究问题价值、方法新意、系统价值、证据强度四个分项，并标注兴趣适配：`target_network_autonomy`、`general_ai_system`、`out_of_scope_domain` 或 `unclear`；同时根据作者区、机构脚注、邮箱域名、致谢、项目页或摘要上下文提取发表单位线索，并把机构名称翻译成中文，无法确认时必须说明“单位线索不足”。
- 服务端按同一套权重和兴趣适配加减分计算周报内部评分，再按“周报入选阈值”和“保底入选篇数”筛选；发布正文里统一展示为“阅读价值评分”。网络自治/电信方向会小幅上浮，通用 AI/Agent 方法轻微上浮；方向不明只轻微后移。只有主问题明确落在医学、生命科学、地理、游戏、推荐系统等专用垂直领域，且看不出可迁移通用方法时，才按专用非目标领域轻度降权；不再设置封顶，质量很高的论文仍可进入推荐区。
