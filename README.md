...
Paper Insight 是一个本地优先的论文推荐 Web 应用，用来持续同步 arXiv 最新论文，并通过 BigModel GLM-5.2 Anthropic 兼容接口对候选论文做分析、打分和总结。
...
- 支持设置“最低达标数”：首轮达标论文超过该数时全部保留；首轮不足时自动启动下一轮候选查询和分析，达到最低达标数后停止追加分析。
- 支持针对单个推荐列表生成“每周高价值论文阅读清单”，输出可复制或下载的 Markdown，便于手动发布到洞察网站。
...
7. 如果首轮阈值以上论文数量低于“最低达标数”，应用会从下一批候选开始新一轮查询和分析；一旦达到最低达标数，就停止继续分析追加候选，而不是从低分论文里补足。
8. 如果某篇论文分析失败，点击“重试分析”会从失败的当前论文继续，不重做已经成功的论文。
9. 分析完成后生成新的推荐列表。
10. 打开某个推荐列表后，可以点击“生成发布版周报”，把该列表中的推荐论文整理成发布版 Markdown。
...
论文推荐必须使用 BigModel GLM-5.2 Anthropic 兼容接口。应用不会使用本地关键词规则兜底。
...
如果 LLM 调用失败、结果缺字段、超时或没有返回某篇论文的分析，服务端会返回错误。应用不会自动无限重试，用户可以在界面上手动确认是否重试；重试会保留已成功分析的结果，并从当前失败论文继续。
...
每个推荐列表都可以生成一份“每周高价值论文阅读清单”。清单只基于当前列表里的推荐论文，不重新查询 arXiv、不重新筛选论文、不改变推荐阈值。

生成阅读清单前，服务端会为列表中的论文尝试抓取 arXiv HTML 原文并做本地缓存。报告优先基于原文摘录生成“内容、方法与结果”和“局限与适用约束”；抓取失败的论文会继续使用摘要和已有分析。

阅读清单由 LLM 生成 Markdown，标题格式为：

```text
YYYY 年 M 月第 N 周高价值论文阅读清单
```

输出内容包括：

- YAML 前置元数据，便于发布到洞察网站。
- 报告导读，概括本周高价值论文、特别值得关注的文章，以及对华为 ADN 网络研究最有价值的方向。
- 本周趋势判断，提炼多篇论文共同呈现出的研究趋势、成熟度风险和对 ADN 的关联。
- 本周必读、值得跟进、快速扫读三个层级。
- 每篇论文的推荐分、符合的评分维度、内容方法与结果、ADN 启发与阅读价值、局限与适用约束。
- 推荐阅读顺序和完整论文清单；完整清单使用“论文、一句话介绍、推荐分、符合维度、阅读级别、链接”，不展示内部阈值或方向列。

## API Key

API Key 可以在页面左上角齿轮设置里输入。应用固定使用 BigModel GLM-5.2 的 Anthropic Messages 兼容接口。Key 只保存在当前浏览器会话的 `sessionStorage`，不会写入项目文件。
