...
- 内容修正仍不能解决论据问题时，才将论文作为单篇内容质量排除并尝试增补；Trace 必须分别记录格式恢复、内容修正、处理失败和内容排除四种结果。
...
- 自动重试一次。
- Trace 分别记录等待和调用耗时。
- 重试后仍失败属于模型调用故障，当前阶段和 Job 必须 fail closed；不得伪装成论文内容失败，也不得据此排除论文。
...
Evidence、Review 或 Calibration 的内容校验或结构化修正后仍失败：
