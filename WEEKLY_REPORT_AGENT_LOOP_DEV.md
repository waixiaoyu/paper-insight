...
- 继续修正：在完整 Trace 中记录管理员决定后，对相同目标再执行一次定向修正和复检；再次失败时仍回到管理员处理。
- 退出任务：以 `reject` 结束，保留当前 Trace、未发布 Markdown 和原因。
- 跳过论文：仅适用于有明确 paperId 的问题。排除该论文、记录原因，并从 reserveCandidates 增补；候选耗尽时按已有合格论文继续。跳过后必须重新执行受影响的后续阶段，不能复用旧的跨论文产物。
