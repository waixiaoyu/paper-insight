        ...
        },
        {
          ...
          content: JSON.stringify({
            ...
            instruction: [
              ...
              "每篇入选论文正文条目都必须展示「阅读价值评分」，并说明分别符合哪些评分维度。阅读价值评分来自输入 readingListReview.score；评分维度来自输入 readingListReview 或 analysis.dimensionDetails / analysis.scores，包括研究问题价值、方法新意、系统价值、证据强度；写出高匹配维度及其分项分，必要时指出较弱维度。analysis.industryTags 里的 ICT、电信、网络自治等产业/方向标签可以作为标签或方向信号展示，但不要写成评分维度，也不要把方向匹配解释为高分依据。",
              "每篇入选论文都必须在正文条目里写「发表单位」，且发表单位必须用中文展示。优先使用 readingListReview.affiliations 和 readingListReview.affiliationEvidence；如果输入里的机构名仍是英文，必须翻译成中文后再写入正文条目。常见大学、公司、研究机构使用通行中文译名；不确定标准译名时用「英文原名（中文意译）」格式。如果 affiliations 是「单位线索不足」，仍必须写成「发表单位：单位线索不足（依据：...）」并解释缺少原文作者区、机构脚注、邮箱域名、致谢或项目页线索。不要把作者姓名误写成单位，不要无依据猜测机构。",
              "不要引用原始推荐列表的旧分数、旧排序或旧推荐/隐藏结论。当前输入论文已经完成内部筛选，Markdown 中的阅读价值评分、阅读层级和排序只依据输入的 readingListReview 结果；不要在发布正文中写“复评”“复评分”这类内部流程词。",
              ...
              "不要在发布内容中体现内部筛选阈值、推荐阈值或复评流程；但每篇入选论文正文条目必须展示自己的阅读价值评分和符合的评分维度。",
              "完整论文清单放在最后，表格列为：论文、一句话介绍、阅读级别、链接。不要在完整论文清单里放发表单位列、阅读价值评分列、符合维度列或方向列；发表单位、评分和维度只在每篇论文正文条目里展示。一句话介绍要概括文章做了什么或为什么值得关注。",
